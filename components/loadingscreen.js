...
// This component is automatically displayed by Next.js whenever a route is loading.
export default function Loading() {
    // The complex clip-path polygon data provided. 
    // This defines the starburst shape.
    const starburstClipPath = "polygon(100% 50%,92.47% 56.73%,97.55% 65.45%,88.31% 69.52%,90.45% 79.39%,80.41% 80.41%,79.39% 90.45%,69.52% 88.31%,65.45% 97.55%,56.73% 92.47%,50% 100%,43.27% 92.47%,34.55% 97.55%,30.48% 88.31%,20.61% 90.45%,19.59% 80.41%,9.55% 79.39%,11.69% 69.52%,2.45% 65.45%,7.53% 56.73%,0% 50%,7.53% 43.27%,2.45% 34.55%,11.69% 30.48%,9.55% 20.61%,19.59% 19.59%,20.61% 9.55%,30.48% 11.69%,34.55% 2.45%,43.27% 7.53%,50% 0%,56.73% 7.53%,65.45% 2.45%,69.52% 11.69%,79.39% 9.55%,80.41% 19.59%,90.45% 20.61%,88.31% 30.48%,97.55% 34.55%,92.47% 43.27%)";
    ...
    return (
        // 1. Full screen overlay: fixed, dark semi-transparent, centered content
        <div className="food-texture fixed inset-0 z-50 flex items-center justify-center w-full h-full backdrop-blur-sm">
            
            {/* 2. The Starburst Container - Now rotates slowly (28s duration) */}
            <div
